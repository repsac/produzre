...
class TestRealize:
    ...
    @staticmethod
    def _after_chord_tone(degree, acc=0):
        # Snapping only fires after a consonant note; the opening note, the
        # final note, and the tonic always keep their written pitch.
        return Theme(
            name="t", role=ThemeRole.MELODY, length_beats=3.0,
            events=(ThemeEvent(0.0, 1.0, 3), ThemeEvent(1.0, 1.0, degree, acc), ThemeEvent(2.0, 1.0, 5)),
            base_register=(60, 72),
        )

    def test_semitone_clash_snaps_to_chord_tone(self):
        # After E, F (deg 4) over I (C E G) snaps to E.
        notes = realize_theme(self._after_chord_tone(4), slots("I"), key="C", mode="major")
        assert notes[1].pitch % 12 == 4
        assert notes[1].snapped
        # The opening note is never snapped, even when it clashes.
        opening = realize_theme(one_note_theme(degree=4), slots("I"), key="C", mode="major")
        assert opening[0].pitch % 12 == 5 and not opening[0].snapped

    def test_color_tone_survives_in_rock(self):
        # b3 over I stays put in rock (blue note), snaps in classical.
        rock = realize_theme(self._after_chord_tone(3, -1), slots("I"),
                             key="C", mode="major", genre="rock")
        assert rock[1].pitch % 12 == 3 and not rock[1].snapped
        plain = realize_theme(self._after_chord_tone(3, -1), slots("I"),
                              key="C", mode="major", genre="classical")
        assert plain[1].snapped
